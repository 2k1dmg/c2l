...
'Р 8.2+ (жчшщь-ёюяь)': function(text) {
	...
	.replace(/ъ([оуаыиэ])/g, (found,p1) => '\''+'ouayiě'['оуаыиэ'.indexOf(p1)])
	.replace(/([жчшщ])(ь)?([иеёюя])?/g, (found,p1,p2,p3) => p1+(p2=='ь'&&p3?'j':'')+(p3?'ieoua'['иеёюя'.indexOf(p3)]:''))
	.replace(/([бвгджзклмнпрстфхцчшщийь])х/g, '$1ch')
	.replace(/[а-яё]/g, (found) => ['a','b','v','g','d','e','ō','ž','z','i','j','k','l','m','n','o','p','r','s','t','u','f','h','c','č','š','ħ','','y','ì','ě','ū','ā']['абвгдеёжзийклмнопрстуфхцчшщъыьэюя'.indexOf(found)]);
}
```
