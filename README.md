Для транслитерации кириллицы в латиницу нужно два (~~или три~~) алфавита на латинице. Первый с диакритическими знаками для записи имен спортсменов и политиков в СМИ, географических названий и так далее. Второй для случаев если нельзя использовать первый. Который состоит только из букв основной латиницы (ASCII). Например для записи имен в банковских система. Пример первого: «**Р 2б/в** (й-y, ы-î, х-x)», второго: «**Р 2 ASCII 2** (й-y, ы-y, х-kh)».

?Третий «ASCII 6 (й-j, ы-y, х-ch, ш-x)» можно использовать если есть только клавиатура с основной латиницей.

Нужно выбрать один из вариантов с **h** или **h/ch/kh** вместо кириллической **х**, и стоит ли добавлять **ď ľ ń ŕ ś ť ź**. Один из вариантов «**Р 2ва/б**» или «**Р 2ва/ба**» без латинской **x** вместо кириллической **х**, потому что такой варинант мало популярен среди других языков.  
Выбранные вариант будет называться "SCUKEN" (или как-нибудь по другому) без цифры.  

Мой выбор: «**Р 2в**». Так же «**Р 4**» с **щ** - **ħ** из-за того что буквы **š** и **ŝ** похожи друг а друга.
